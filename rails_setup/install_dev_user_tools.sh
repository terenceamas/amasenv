...
# Per-user development tool bootstrap for a server whose system dependencies
# were already installed by rails_setup/install.sh.
...
for command_name in git curl bash; do
    command -v "${command_name}" >/dev/null 2>&1 \
        || die "${command_name} is required. Run rails_setup/install.sh first."
done
...
Install Rails through the project's Gemfile whenever possible.
==============================================================================
EOF
